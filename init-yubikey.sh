...
function __setup_efi_external_ca() {
    ...
    cp pkCA/private/pk.key.pem ../efi.tmpfs/efi.pk-host.priv.pem
    cp pkCA/csr/pk.csr.pem ../efi.tmpfs/efi.pk-host.csr.pem
    cp pkCA/certs/pk.cert.pem ../efi.tmpfs/efi.pk-host.crt.pem
    openssl rsa -in ../efi.tmpfs/efi.pk-host.priv.pem -passin pass:"$JANUS_EFI_PASSPHRASE_PK" -pubout -out ../efi.tmpfs/efi.pk-host.pub.pem

    cp kekCA/private/kek.key.pem ../efi.tmpfs/efi.kek-host.priv.pem
    cp kekCA/csr/kek.csr.pem ../efi.tmpfs/efi.kek-host.csr.pem
    cp kekCA/certs/kek.cert.pem ../efi.tmpfs/efi.kek-host.crt.pem
    openssl rsa -in ../efi.tmpfs/efi.kek-host.priv.pem -passin pass:"$JANUS_EFI_PASSPHRASE_KEK" -pubout -out ../efi.tmpfs/efi.kek-host.pub.pem

    openssl genrsa -aes256 -out ../efi.tmpfs/efi.db-user.priv.pem -passout pass:"${JANUS_EFI_PASSPHRASE_USER}" 2048
    openssl rsa -in ../efi.tmpfs/efi.db-user.priv.pem -passin pass:"$JANUS_EFI_PASSPHRASE_USER" -pubout -out ../efi.tmpfs/efi.db-user.pub.pem

    openssl req -config kekCA.cnf \
        -new -sha256 -key ../efi.tmpfs/efi.db-user.priv.pem \
        -out ../efi.tmpfs/efi.db-user.csr.pem \
    ...
    openssl ca -config kekCA.cnf \
        -extensions efi_db \
        -days 730 -notext -md sha256 \
        -in ../efi.tmpfs/efi.db-user.csr.pem \
        -out ../efi.tmpfs/efi.db-user.crt.pem \
        -passin pass:"$JANUS_EFI_PASSPHRASE_KEK" \
        -batch
    ...
    while (($#)) ; do
        _passphrase=$(( ${#JANUS_MACHINES} - $# + 1 ))
        _passphrase="$(echo -e "$JANUS_EFI_PASSPHRASE_MACHINES" | cut -f $_passphrase)"

        openssl genrsa -aes256 -out ../efi.tmpfs/efi.db-machine-$1.priv.pem -passout pass:"${_passphrase}" 2048
        openssl rsa -in ../efi.tmpfs/efi.db-machine-$1.priv.pem -passin pass:"$_passphrase" -pubout -out ../efi.tmpfs/efi.db-machine-$1.pub.pem

        openssl req -config kekCA.cnf \
            -new -sha256 -key ../efi.tmpfs/efi.db-machine-$1.priv.pem \
            -out ../efi.tmpfs/efi.db-machine-$1.csr.pem \
            -passin pass:"${_passphrase}" \
        ...
        openssl ca -config kekCA.cnf \
            -extensions efi_db \
            -days 730 -notext -md sha256 \
            -in ../efi.tmpfs/efi.db-machine-$1.csr.pem \
            -out ../efi.tmpfs/efi.db-machine-$1.crt.pem \
            -passin pass:"$JANUS_EFI_PASSPHRASE_KEK" \
            -batch
    ...
    cp "$JANUS_EFI_FACTORY_SRC/PK.esl" efi.pk-factory.esl
    cp "$JANUS_EFI_FACTORY_SRC/KEK.esl" efi.kek-factory.esl
    cp "$JANUS_EFI_FACTORY_SRC/db.esl" efi.db-factory.esl
    cp "$JANUS_EFI_FACTORY_SRC/dbx.esl" efi.dbx-factory.esl

    for _file in *.crt.pem; do
        cert-to-efi-sig-list -g "$(< efi.guid.txt)" "$_file" "${_file%.crt.pem}.esl"
    done

    cat efi.kek-factory.esl efi.kek-host.esl > efi.kek.esl
    cat efi.db-factory.esl efi.db-user.esl efi.db-machine-*.esl > efi.db.esl
    cat efi.dbx-factory.esl > efi.dbx.esl

    mkfifo privkey.fifo
    openssl rsa -in efi.pk-host.priv.pem -passin pass:"$JANUS_EFI_PASSPHRASE_PK" -out privkey.fifo &
    sign-efi-sig-list -k privkey.fifo -c efi.pk-host.crt.pem PK efi.pk-host.esl efi.pk-host.auth &
    wait
    rm privkey.fifo

    mkfifo privkey.fifo
    openssl rsa -in efi.pk-host.priv.pem -passin pass:"$JANUS_EFI_PASSPHRASE_PK" -out privkey.fifo &
    sign-efi-sig-list -k privkey.fifo -c efi.pk-host.crt.pem KEK efi.kek.esl efi.kek.auth &
    wait
    rm privkey.fifo

    mkfifo privkey.fifo
    openssl rsa -in efi.kek-host.priv.pem -passin pass:"$JANUS_EFI_PASSPHRASE_KEK" -out privkey.fifo &
    sign-efi-sig-list -k privkey.fifo -c efi.kek-host.crt.pem db efi.db.esl efi.db.auth &
    wait
    rm privkey.fifo

    mkfifo privkey.fifo
    openssl rsa -in efi.kek-host.priv.pem -passin pass:"$JANUS_EFI_PASSPHRASE_KEK" -out privkey.fifo &
    sign-efi-sig-list -k privkey.fifo -c efi.kek-host.crt.pem dbx efi.dbx.esl efi.dbx.auth &
    wait
    rm privkey.fifo
...
function __export_efi() {
    ...
    cp efi.tmpfs/efi.pk-host.auth export.tmpfs/efi/import/pk.auth
    cp efi.tmpfs/efi.kek.auth export.tmpfs/efi/import/kek.auth
    cp efi.tmpfs/efi.db.auth export.tmpfs/efi/import/db.auth
    cp efi.tmpfs/efi.dbx.auth export.tmpfs/efi/import/dbx.auth
...
function __finalize_backup() {
    ...
    while (($#)); do
        printf '%s' \
            "$(echo -e "$JANUS_EFI_PASSPHRASE_MACHINES" | cut -f $(( ${#JANUS_MACHINES} - $# + 1 )))" \
            > backup.tmpfs/efi/efi.db-machine-$1.passphrase.txt
        shift
    done

    mkdir -p backup.tmpfs/efi/{pk,kek,db,factory,import}

    mv backup.tmpfs/efi/*.auth backup.tmpfs/efi/import
    mv backup.tmpfs/efi/efi.*-factory.esl backup.tmpfs/efi/factory
...
JANUS_TOUCH=false
JANUS_EFI=false
JANUS_EFI_FACTORY_SRC="efi-sb-keys.factory"
